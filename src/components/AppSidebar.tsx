...
const AppSidebar: React.FC<AppSidebarProps> = ({ activeSection, onSectionChange }) => {
  ...
  const menuItems = [
    ...
    { 
      ...
      hasSubmenu: true,
      submenu: [
        { id: 'operators', label: 'Operators', icon: Users, isRoute: true },
        { id: 'nodes', label: 'Nodes', icon: Smartphone, isRoute: true },
        { id: 'agents', label: 'Agents', icon: Bot, isRoute: true },
        { id: 'checks', label: 'Checks', icon: CheckCircle, isRoute: true }
      ]
    },
  ...
  const handleMenuClick = (item: any) => {
    if (item.isRoute) {
      ...
      navigate(`/network?section=${item.id}`, { replace: location.pathname === '/network' });
  ...
  const handleSubMenuClick = (subItemId: string, subItem: any) => {
    if (subItem.isRoute) {
      ...
      navigate(`/network?section=${subItemId}`, { replace: location.pathname === '/network' });
    ...
    return submenu.map((subItem) => {
      ...
      return (
        <SidebarMenuItem key={subItem.id}>
          <SidebarMenuButton
            onClick={() => handleSubMenuClick(subItem.id, subItem)}
        ...
        <div className="flex items-center justify-between p-2">
          {!isCollapsed ? (
            <div className="flex-1">
              ...
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">Global Testing Network</p>
            </div>
          ) : null}
                ...
                return (
                  <React.Fragment key={item.id}>
                    ...
                    {isCollapsed && item.hasSubmenu && item.submenu && 
                      renderCollapsedSubmenuItems(item.submenu)
                    }
                  </React.Fragment>
                );
              })}
